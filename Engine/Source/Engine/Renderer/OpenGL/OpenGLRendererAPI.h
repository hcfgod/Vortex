...
namespace Vortex
{
	...
	public:
		...
		Result<void> DeleteFramebuffers(uint32_t count, const uint32_t* fbos) override;
	...
	private:
		...
		uint32_t ConvertFramebufferTarget(uint32_t target) const;
		uint32_t ConvertFramebufferAttachment(uint32_t attachment) const;
	};
}
